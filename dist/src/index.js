...
config_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_COMPILE, async (args, hre, runSuper) => {
    ...
    const { writeValidations } = await Promise.resolve().then(() => __importStar(require('@openzeppelin/hardhat-upgrades/dist/utils/validations')));
    // TODO: patch input
    const { output, solcBuild } = await runSuper();
    const { isFullSolcOutput } = await Promise.resolve().then(() => __importStar(require('@openzeppelin/hardhat-upgrades/dist/utils/is-full-solc-output')));
